    ...
    for file_path in listdir:
        ...
        while ret:
            ...
            for i in range(boxes.shape[0]):
                ...
                all_face_images.append(base64.b64encode(tmp_cut_face))
                all_face_results.append(show_info)
            ...
            #     # switch base64 ========== tmp_cut_face.tobytes()
            #     res = requests.post(url=url, data={"upData": json.dumps(upDate)
            #         , "face_results": all_face_results
            #         , "files": all_face_images})
            #     print(res.text)
...
def inferenceHtptsVideo():
    ...
    url = 'rtsp://admin:Xhs88283111@192.168.2.5/Streaming/Channels/1'
    ...
    while ret:
        ...
        for i in range(boxes.shape[0]):
            ...
            upDate["image"] = np.array(cv2.imencode('.png', tmp_cut_face)[1]).tobytes()
