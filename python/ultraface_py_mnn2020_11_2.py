...
def inferenceHtptsVideo(dist_q, frame,interpreter,input_tensor,session):
...
def faceComparison(face_images,windowName):
    ...
    for image in face_images:
        ...
        # 读取对应编号的图片
        print(len(names_list[random_people_index]))
...
def image_put(q, user, pwd, ip, channel=1):
    ...
    while cap.isOpened():
        ...
        if (time.time() - start_time) > 100:
            ...
            cap = cv2.VideoCapture("rtsp://admin:flm2019hb@192.168.101.107:554")
        # 读取摄像头数据
        inferenceHtptsVideo(q, cap.read()[1], interpreter, input_tensor, session)
